...
const EnhancedTableToolbar = (props) => {
  ...
  const handleClose = (event, reason) => {
    if (reason === 'clickaway') {
      ...
      setInterval(() => setAlert({ ...alert, open: false }), 3000);
    }
  };
...
export default function EnhancedTable({
  ...
  const switchFilter = () => {
    ...
    const andriodApps = rows.filter((row) =>
      andriodChecked ? row.platforms.includes('Andriod') : null
    );
    const softwareApps = rows.filter((row) =>
      softwareChecked ? row.service === 'Custom Software' : null
    );

    if (!websiteChecked && !iOSChecked && !andriodChecked && !softwareChecked) {
      return rows;
    } else {
      ...
      let newRows2 = newRows.concat(
        andriodApps.filter((item) => newRows.indexOf(item) < 0)
      );
      ...
      <Paper className={classes.paper} elevation={0}>
        ...
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              {stableSort(
                priceFilter(switchFilter()).filter((row) => row.search),
        ...
        <TablePagination
          ...
          component='div'
          count={
            priceFilter(switchFilter()).filter((option) => option.search).length
          }
